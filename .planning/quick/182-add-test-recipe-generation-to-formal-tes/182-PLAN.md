...
must_haves:
  truths:
    ...
    - "extractPropertyDefinition TLA+ regex uses $ with multiline flag (not \\Z which is invalid JS)"
    - "printSummary() outputs recipe count alongside stubs count for observability"
  artifacts:
...
- In `printSummary()`, add a recipe count line alongside the existing stubs count output. After the line that prints stubs count (e.g., `console.log('[formal-test-sync] Stubs: ...')`), add: `console.log('[formal-test-sync] Recipes: ' + recipeCount + ' generated');` — pass the recipe count from `generateStubs()` return data. This ensures both dry-run and normal runs show recipe observability.
...
Run: `node bin/formal-test-sync.cjs --dry-run 2>&1 | grep -i recipe` — should show recipe count line from printSummary().
  </verify>
  <done>
...
- TLA+ regex uses valid JS syntax ($ not \Z) with multiline flag
- printSummary() shows recipe count for both dry-run and normal execution
- First-property-only behavior for multi-property gaps is documented in code comment
</success_criteria>

<output>
After completion, create `.planning/quick/182-add-test-recipe-generation-to-formal-tes/182-SUMMARY.md`
</output>
