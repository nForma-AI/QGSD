...
## Current State: v0.42 Complete — Planning Next Milestone

v0.42 (Deep coderlm Solve Integration) shipped 2026-04-10. All 11 requirements satisfied across 4 phases, 9 plans. Audit: PASSED. The solve loop now makes call-graph-driven decisions via a fail-open coderlm adapter at 8 integration points. Next milestone: `/nf:new-milestone`.

## Shipped: v0.42 — Deep coderlm Solve Integration (2026-04-10)
...
**Shipped:** 11/11 requirements satisfied across 4 phases, 9 plans. Audit: PASSED.

**Key features shipped:**
- LRU cache adapter (`bin/coderlm-cache.cjs`) with 100-entry capacity, 5-min TTL, wired into all 6 query methods; metrics emitted to stderr per solve session (CADP-01/02/03)
- Symbol-level edge discovery via `getImplementationSync` + `LAYER_SYMBOL_MAP` replacing file-name string matching in `queryEdgesSync()`; coderlm reindex after autoClose() (CDIAG-01/04)
- R→F seed files via `getImplementationSync`/`getCallersSync` in close-formal-gaps dispatch; F→T recipe enrichment via `findTests`/`peek` patterns (CREM-01/02)
- Heatmap hot-zone ranking incorporates `Math.log1p(calleeCount)` alongside git churn (CREM-03); reverse discovery candidates carry `caller_count` and `dead_code_flag` (CREM-04)
- `formal-scope-scan.cjs` Layer 2.5 backward call-graph walk discovers transitive formal model dependencies (CDIAG-02); `expandWithCallGraph()` prevents incorrect layer skips (CDIAG-03)
...
**Shipped:** 16/16 requirements satisfied across 4 phases, 7 plans. Audit: PASSED.

**Key features shipped:**
- `/nf:debug` absorbs model-driven-fix formal pipeline (4-step: discovery, reproduction, refinement via Loop 1, constraint extraction) as single entry point for formal-model-driven bug investigation
- Automatic Haiku-based task classification at quick.md Step 2.7 (bug_fix/feature/refactor with confidence scoring) with scope-contract.json persistence
- Bug fix routing: tasks classified as bug_fix (confidence >= 0.7) automatically dispatched through `/nf:debug` before executor, with constraints injected into executor prompt
- Loop 2 pre-commit simulation gate (simulateSolutionLoop + onTweakFix) in both quick.md and execute-phase.md executors, fail-open by default with --strict override
- `/nf:model-driven-fix` deprecated with deprecation shim, solve-remediate b_to_f rewired to `/nf:debug`
...
**Goal:** Evolve formal models from descriptive (CI validation gate) to prescriptive (models explain bugs, constrain the solution space, and verify fixes before they're written).

**Shipped:** 20/20 requirements satisfied across 5 phases, 11 plans. Audit: PASSED.

**Key features shipped:**
- Bug-to-model lookup with semantic matching, TLC/Alloy model checker execution, and persistent gap tracking (bug-model-gaps.json)
- TLA+/Alloy constraint extraction with plain-English rendering for quorum worker prompt injection
- Debug command formal model consultation (Step A.5) with FORMAL verdict row and gap persistence
- Model refinement 6-phase prescriptive cycle with inverted verification (model must FAIL to confirm bug capture)
- B→F solve layer (20th layer) with three-bucket classification and autonomous gap/blind-spot routing
- Cross-model regression prevention via 2-hop proximity-neighbor verification before declaring fix done
...
- ✓ R->F remediation enriched with coderlm seed-files: `close-formal-gaps --seed-files` injects `getImplementationSync`/`getCallersSync` results into Step 5 spec generation; `solve-remediate` 3a dispatches per-requirement when seeds present (no cross-contamination); fail-open to batch dispatch when coderlm unavailable (CREM-01) — v0.42 (Phase 055)
- ✓ F->T remediation enriched with coderlm recipe patterns: `formal-test-sync --enrich-recipes` uses `findTests`/`peek` to populate recipe JSON sidecars with observed assertion patterns; idempotent (skips recipes with existing patterns); `solve-remediate` 3b runs enrichment as Phase 1a; fail-open when coderlm unavailable (CREM-02) — v0.42 (Phase 055)
- ✓ Git heatmap hot-zone ranking enriched with callee counts: `computePriority()` accepts `calleeCount` (Math.log1p weighted); `sweepGitHeatmap()` queries `getCallersSync` per hot-zone file via `_activeAdapter`, re-sorts by updated priority; fail-open to git churn ranking when coderlm unavailable (CREM-03) — v0.42 (Phase 056)
- ✓ Reverse discovery (C->R, T->R) enriched with caller counts: `sweepCtoR()` and `sweepTtoR()` add `caller_count` and `dead_code_flag` (true when caller_count===0) to each candidate; solve report annotates "(0 callers — likely dead code)" for zero-caller files; fail-open when coderlm unavailable (CREM-04) — v0.42 (Phase 056)
- ✓ `formal-scope-scan.cjs` Layer 2.5 call-graph backward walk: `discoverViaCallGraph()` walks `getCallers()` from changed files and matches returned callers against formal module `source_files` globs; new matches marked `matched_by: 'call_graph'`; fail-open when coderlm unavailable (CDIAG-02) — v0.42 (Phase 057)
- ✓ `solve-incremental-filter.cjs` transitive dependency expansion: `expandWithCallGraph()` post-processes DOMAIN_MAP result by querying callers of changed files and adding any matching layers to `affectedSet`; monotone-safe (add-only); CLI accepts `--adapter-host`/`NF_CODERLM_HOST`; fail-open (CDIAG-03) — v0.42 (Phase 057)
...
| Symbol hint derived from requirement ID (stripped dashes, lowercase) for coderlm queries | Requirement text is not always available in the dispatch context; ID-derived hint (CREM-01 → crem01) provides a usable function-name fallback | Phase 055 — CREM-01 |
| Per-requirement R->F dispatch when seeds present | Individual close-formal-gaps calls prevent caller context from bleeding across unrelated requirements (cross-contamination); unseeded requirements batch as before | Phase 055 — CREM-01 |
| Recipe enrichment runs async AFTER main() completes (async IIFE tail) | Wrapping main() would break process.exit() timing and interfere with synchronous test assertions; tail pattern preserves all existing behavior | Phase 055 — CREM-02 |
| Idempotency check on assert_patterns.length > 0 (not test_files.length) | Allows partial enrichment re-runs: a recipe may have test_files but zero extracted patterns (e.g., empty peek result) — checking assert_patterns ensures incomplete enrichment is retried | Phase 055 — CREM-02 |
| Math.log1p(calleeCount) for heatmap priority weighting | Sublinear growth avoids priority explosion on very-high-caller files while still giving meaningful boost (10 callers → 1.4x, 100 callers → 1.6x); neutral (1.0x) for 0-caller files preserves pre-integration behavior | Phase 056 — CREM-03 |
| Module-level _activeAdapter for sweepGitHeatmap() access | sweepGitHeatmap() is a top-level function while _solveAdapter is scoped inside solve(); module-level variable (matching formalTestSyncCache pattern) avoids refactoring all callers | Phase 056 — CREM-03 |
| dead_code_flag = caller_count === 0 (not < threshold) | Binary flag avoids an arbitrary threshold that would need calibration; 0 callers is the definitive signal — nothing calls this module anywhere in the indexed graph | Phase 056 — CREM-04 |
| Layer 2.5 inserted between Layer 2 (proximity) and Layer 3 (semantic fallback) | Keeps existing layer ordering; add-only monotone pattern guarantees EventualConvergence invariant holds regardless of call-graph results | Phase 057 — CDIAG-02 |
| expandWithCallGraph is a post-processor (not inline in DOMAIN_MAP) | Preserves existing DOMAIN_MAP regex matching as the primary signal; call-graph expansion only adds layers, never overrides or removes existing results | Phase 057 — CDIAG-03 |

## Current Status

v0.42 shipped 2026-04-10. 42 milestones completed (v0.1–v0.42). The solve loop is now call-graph-aware via fail-open coderlm integration at 8 points. Next milestone: run `/nf:new-milestone` to start.

---
*Last updated: 2026-04-10 after v0.42 milestone completion (Deep coderlm Solve Integration — 11/11 requirements, audit PASSED)*
