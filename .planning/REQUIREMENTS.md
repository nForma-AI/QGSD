...
- [ ] **TOKN-03**: User can view a token usage dashboard aggregating cost per milestone, phase, and slot from token-usage.jsonl
...
- [ ] **MEMP-01**: User's accumulated decisions, rejected approaches, and partial findings persist across compaction via structured state beyond STATE.md
...
- [ ] **MEMP-03**: User benefits from error resolution memory — tricky bug fixes persist as searchable symptom/root-cause/fix patterns via subagent memory
- [ ] **MEMP-04**: User benefits from quorum decision memory — debate rationale ("chose X over Y because Z") persists across compaction and sessions

### Continuous Learning

- [ ] **LRNG-01**: User benefits from automatic error resolution extraction — symptom-to-root-cause-to-fix patterns extracted into a searchable catalog at session boundaries
- [ ] **LRNG-02**: User corrections to Claude's approach are automatically recorded as learned patterns for future sessions
...
- [ ] **LRNG-04**: User benefits from a failure catalog tracking failed approaches with confidence scores to prevent re-attempting dead ends
