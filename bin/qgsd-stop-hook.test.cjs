...
// Tests for the QGSDStopHook TLA+ specification — SPEC-01.
//
...
// Test 3: model-registry.json has entry for .planning/formal/tla/QGSDStopHook.tla with update_source=manual
...
const TLA_FILE   = path.join(ROOT, '.planning', 'formal', 'tla', 'QGSDStopHook.tla');
...
test('QGSDStopHook.tla and MCStopHook.cfg exist on disk', () => {
  assert.ok(fs.existsSync(TLA_FILE), '.planning/formal/tla/QGSDStopHook.tla must exist');
...
test('model-registry.json has entry for .planning/formal/tla/QGSDStopHook.tla with update_source=manual', () => {
  assert.ok(fs.existsSync(REGISTRY), '.planning/formal/model-registry.json must exist');
  const registry = JSON.parse(fs.readFileSync(REGISTRY, 'utf8'));
  const entry = (registry.models || {})['.planning/formal/tla/QGSDStopHook.tla'];
  assert.ok(entry, 'model-registry.json must have an entry for .planning/formal/tla/QGSDStopHook.tla');
