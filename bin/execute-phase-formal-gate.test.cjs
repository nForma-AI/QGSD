...
// STRUCTURAL tests are RED until Plan 02 updates core/workflows/execute-phase.md
// and Plan 04 installs it to ~/.claude/nf/workflows/execute-phase.md.
...
test('extractModules: single entry returns module name', () => {
  const input = ['{"module":"quorum","path":".planning/formal/spec/quorum/invariants.md"}'];
  assert.strictEqual(extractModules(input), 'quorum');
});

test('extractModules: two entries returns comma-separated names', () => {
  const input = [
    '{"module":"quorum","path":".planning/formal/spec/quorum/invariants.md"}',
    '{"module":"breaker","path":".planning/formal/spec/breaker/invariants.md"}',
  ];
...
// These tests read the INSTALLED execute-phase.md from ~/.claude/nf/workflows/execute-phase.md
// Plan 04 runs the installer — until then, this file lacks the formal gate content.

const INSTALLED_EXECUTE_PHASE = path.join(os.homedir(), '.claude', 'nf', 'workflows', 'execute-phase.md');
...
test('execute-phase.md contains run-formal-check.cjs invocation', () => {
  assert.match(
    ...
    'Pattern not found: expected "run-formal-check.cjs" in ~/.claude/nf/workflows/execute-phase.md'
  );
});
...
test('execute-phase.md contains FORMAL_CHECK_RESULT extraction pattern', () => {
  assert.match(
    executePhaseContent,
    /FORMAL_CHECK_RESULT/,
    'Pattern not found: expected "FORMAL_CHECK_RESULT" in ~/.claude/nf/workflows/execute-phase.md'
  );
});
...
test('execute-phase.md contains counterexample_found status routing', () => {
  assert.match(
    ...
    'Pattern not found: expected "counterexample_found" in ~/.claude/nf/workflows/execute-phase.md'
  );
});
...
test('execute-phase.md contains counterexample_override override path', () => {
  assert.match(
    ...
    'Pattern not found: expected "counterexample_override" in ~/.claude/nf/workflows/execute-phase.md'
  );
});
