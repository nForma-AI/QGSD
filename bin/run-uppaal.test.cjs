...
test('run-uppaal.cjs writes inconclusive result to NDJSON when verifyta not found (UPPAAL-02)', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uppaal-test-'));
  fs.mkdirSync(path.join(tmpDir, '.planning', 'formal'), { recursive: true });
  const ndjsonOut = path.join(tmpDir, '.planning', 'formal', 'check-results.ndjson');
  const result = spawnSync(process.execPath, [TOOL_PATH], {
    cwd: tmpDir,
    encoding: 'utf8',
    timeout: 15000,
...
test('run-uppaal.cjs reads runtime_ms bounds from check-results.ndjson (UPPAAL-02)', () => {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'uppaal-test-'));
  fs.mkdirSync(path.join(tmpDir, '.planning', 'formal'), { recursive: true });
  const ndjsonIn = path.join(tmpDir, '.planning', 'formal', 'check-results.ndjson');
...
// Test 5: .planning/formal/uppaal/quorum-races.xml exists and is valid XML
test('.planning/formal/uppaal/quorum-races.xml exists and is valid XML (UPPAAL-01)', () => {
  const xmlPath = path.join(__dirname, '..', '.planning', 'formal', 'uppaal', 'quorum-races.xml');
...
// Test 6: .planning/formal/uppaal/quorum-races.q exists and contains required property queries
test('.planning/formal/uppaal/quorum-races.q exists and contains required property queries (UPPAAL-03)', () => {
  const qPath = path.join(__dirname, '..', '.planning', 'formal', 'uppaal', 'quorum-races.q');
