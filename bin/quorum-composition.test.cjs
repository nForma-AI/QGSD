...
//
// Test 1: .planning/formal/alloy/quorum-composition.als exists
...
// Test 4: model-registry.json has entry for .planning/formal/alloy/quorum-composition.als with update_source=manual
...
const ALS_FILE  = path.join(ROOT, '.planning', 'formal', 'alloy', 'quorum-composition.als');
const JAR_PATH  = path.join(ROOT, '.planning', 'formal', 'alloy', 'org.alloytools.alloy.dist.jar');
...
test('.planning/formal/alloy/quorum-composition.als exists', () => {
  assert.ok(fs.existsSync(ALS_FILE), '.planning/formal/alloy/quorum-composition.als must exist');
});
...
test('model-registry.json has entry for .planning/formal/alloy/quorum-composition.als with update_source=manual', () => {
  assert.ok(fs.existsSync(REGISTRY), '.planning/formal/model-registry.json must exist');
  const registry = JSON.parse(fs.readFileSync(REGISTRY, 'utf8'));
  const entry = (registry.models || {})['.planning/formal/alloy/quorum-composition.als'];
  assert.ok(entry, 'model-registry.json must have an entry for .planning/formal/alloy/quorum-composition.als');
