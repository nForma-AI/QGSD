...
if (!VALID_SPECS.includes(specName)) {
  ...
  try { writeCheckResult({ tool: 'run-audit-alloy', formalism: 'alloy', result: 'fail', metadata: { spec: specName } }); } catch (e) { process.stderr.write('[run-audit-alloy] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
...
try { writeCheckResult({ tool: 'run-audit-alloy', formalism: 'alloy', result: 'pass', metadata: { spec: specName } }); } catch (e) { process.stderr.write('[run-audit-alloy] Warning: failed to write check result: ' + e.message + '\n'); }
process.exit(0);
