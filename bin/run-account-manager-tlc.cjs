...
if (!VALID_CONFIGS.includes(configName)) {
  ...
  const _runtimeMs = 0;
  try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: unknown config in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
...
if (JAVA_HOME) {
  ...
  if (!fs.existsSync(javaExe)) {
    ...
    const _startMs = Date.now();
    const _runtimeMs = 0;
    try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: Java not found in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
    process.exit(1);
  }
} else {
...
if (versionResult.error || versionResult.status !== 0) {
  ...
  const _startMs = Date.now();
  const _runtimeMs = 0;
  try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: Java version check failed in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
...
if (javaMajor < 17) {
  ...
  const _startMs = Date.now();
  const _runtimeMs = 0;
  try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: Java ' + javaMajor + ' < 17 in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
...
if (!jarPath) {
  process.stderr.write(
    '[run-account-manager-tlc] tla2tools.jar not found.\n' +
    '[run-account-manager-tlc] Install: node bin/install-formal-tools.cjs\n'
  );
  const _startMs = Date.now();
  const _runtimeMs = 0;
  try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: tla2tools.jar not found in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
...
if (tlcResult.error) {
  ...
  try { writeCheckResult({ tool: 'run-account-manager-tlc', formalism: 'tla', result: 'error', check_id: 'tla:account-manager', surface: 'tla', property: 'Account manager quorum state machine — MCAM correctness', runtime_ms: _runtimeMs, summary: 'error: TLC invocation failed in ' + _runtimeMs + 'ms', requirement_ids: getRequirementIds('tla:account-manager'), metadata: {} }); } catch (e) { process.stderr.write('[run-account-manager-tlc] Warning: failed to write check result: ' + e.message + '\n'); }
  process.exit(1);
}
