'use strict';
var { f } = require('../../../bin/bench-buggy-extreme-write-skew.cjs');
var failed = 0;
var _i = 0;
...
// Correct: f returns true. Buggy: returns false (no write-write overlap).
assert('write skew detected (T1:read={A,B},write={A} T2:read={A,B},write={B})',
  f(['A', 'B'], ['A'], ['A', 'B'], ['B']) === true,
  'f returned false — write skew anomaly missed (only dirty write checked)');

// Dirty write: T1 writes {A}, T2 writes {A} — write-write overlap → conflict.
assert('dirty write detected', f(['A'], ['A'], ['B'], ['A']) === true, 'got false');

// No conflict: disjoint read and write sets.
assert('no conflict: all disjoint', f(['A'], ['B'], ['C'], ['D']) === false, 'got true');
...
assert('truly disjoint write skew test: no conflict', f(['X','Y'],['X'],['Y','Z'],['Z']) === false,
  'got true');
...
assert('no conflict when writes not in other read sets', f(['K'],['A'],['K'],['B']) === false,
  'got true');
...
assert('write-read anti-dependency: T1 writes K, T2 reads K',
  f([], ['K'], ['K'], []) === true,
  'f returned false — anti-dependency missed');
