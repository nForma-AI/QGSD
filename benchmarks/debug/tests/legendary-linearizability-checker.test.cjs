...
assert('stale read after write is not linearizable', isLinearizable(ops), false);
...
assert('consistent read after write is linearizable', isLinearizable(ops2), true);

// Overlapping operations: read returns 0 before write starts — linearizable (read before write)
var ops3 = [
  {start:1, end:5, type:'write', key:'x', value:1, seqPos:1},
  {start:2, end:3, type:'read',  key:'x', value:0, seqPos:0}
];
assert('overlapping read before write linearizable', isLinearizable(ops3), true);
