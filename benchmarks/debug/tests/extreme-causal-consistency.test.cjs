'use strict';
var { f } = require('../../../bin/bench-buggy-extreme-causal-consistency.cjs');
var failed = 0;
var _i = 0;
...
var b = [{cause: 'W1', effect: 'W2'}];

// VALID: both visible
assert('sees both W1 and W2: consistent', f(['W1', 'W2'], b) === true,
  'got false');
...
assert('sees W2 without W1: inconsistent', f(['W2'], b) === false,
  'f(["W2"],[{cause:W1,effect:W2}]) returned true — ' +
  'effect visible without cause, violation missed (wrong implication direction)');

// VALID: sees neither
assert('sees neither: consistent', f([], b) === true, 'got false');
...
assert('sees only W1: consistent', f(['W1'], b) === true,
  'f(["W1"]) returned false — cause visible without effect is valid, ' +
...
// VALID: all visible
assert('chain all visible: consistent', f(['W1','W2','W3'], chain) === true, 'got false');

// INVALID: W3 visible but W2 missing
assert('chain W3 visible W2 missing: inconsistent', f(['W1','W3'], chain) === false,
  'chain violation missed: W3 visible but W2 not visible');

// INVALID: W2 visible but W1 missing
assert('chain W2 visible W1 missing: inconsistent', f(['W2','W3'], chain) === false,
  'chain violation missed: W2 visible but W1 not visible');

// VALID: W1, W2 visible (W3 not yet propagated)
assert('chain partial W1,W2: consistent', f(['W1','W2'], chain) === true,
...
assert('multi: W2 without W1 inconsistent', f(['W2','W3','W4'], multi) === false,
  'multi-pair violation in first pair missed');

// VALID: W3 visible without W4 (cause without effect is fine)
assert('multi: W3 without W4 valid', f(['W1','W2','W3'], multi) === true,
