#!/usr/bin/env node
// hooks/nf-stop-fan-out.test.cjs — Wave 0 test scaffold for FAN-04
// Integration tests for nf-stop.js ceiling verification with adaptive fan-out
...
// Tests verify that nf-stop.js reads --n N from transcript and verifies correct ceiling.
...
function writeTempTranscript(lines) {
  const tmpFile = path.join(os.tmpdir(), `nf-stop-fan-test-${Date.now()}-${Math.random().toString(36).slice(2)}.jsonl`);
  fs.writeFileSync(tmpFile, lines.join('\n') + '\n', 'utf8');
  return tmpFile;
}

// Helper: spawn nf-stop.js as child process with stdin
...
// JSONL builder helpers (from nf-stop.test.js)
