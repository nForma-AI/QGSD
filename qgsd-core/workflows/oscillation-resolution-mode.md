...
- Instructions: "To implement: [steps]. After committing the fix, run `node ~/.claude/qgsd-bin/qgsd-manage.cjs --reset-breaker` to clear the circuit breaker."
...
- No commits to the oscillating file set until user approves the plan AND runs `node ~/.claude/qgsd-bin/qgsd-manage.cjs --reset-breaker`
