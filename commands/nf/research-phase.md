...
```bash
INIT=$(node ~/.claude/nf/bin/nf-tools.cjs init phase-op "$ARGUMENTS")
```
...
```bash
RESEARCHER_MODEL=$(node ~/.claude/nf/bin/nf-tools.cjs resolve-model nf-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node ~/.claude/nf/bin/nf-tools.cjs roadmap get-phase "${phase_number}")
```
